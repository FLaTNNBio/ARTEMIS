...
BEST_PARAMS = {
    ...
    'feature_k': 20,
    # new MI-related switches
    'mi_mode': 'local_pos',       # none | local_pos | global_batch
...
# TRAIN
# ==============================================================================
def select_mi_subset(mi_mode: str, z1, z2, t1_idx, t2_idx, pos_mask):
    if mi_mode == 'none':
        return None, None
    if mi_mode == 'local_pos':
        if pos_mask.sum().item() == 0:
            return None, None
        z_sel = torch.cat([z1[pos_mask], z2[pos_mask]], dim=0)
        t_sel = torch.cat([t1_idx[pos_mask], t2_idx[pos_mask]], dim=0)
        return z_sel, t_sel
    if mi_mode == 'global_batch':
...
def train_single_simulation(sim_idx: int, data_train: Tuple, data_test: Tuple, device: str, hyperparams: Dict[str, Any]) -> Dict[str, float]:
    ...
    MI_MODE = hyperparams.get('mi_mode', 'local_pos')
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in dl_train:
            ...
            z_mi_det, t_mi_det = select_mi_subset(MI_MODE, z1.detach(), z2.detach(), t1_idx, t2_idx, pos_mask)
            ...
            z_mi, t_mi = select_mi_subset(MI_MODE, z1, z2, t1_idx, t2_idx, pos_mask)
...
def run_setting(setting_name: str, params: Dict[str, Any], X_tr, T_tr, YF_tr, mu0_tr, mu1_tr, X_te, T_te, YF_te, mu0_te, mu1_te, n_sims: int):
...
def build_mi_ablation_configs(best_params: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    p['mi_mode'] = 'local_pos'
    p['mi_use_schedule'] = True
    configs['full_local_mi'] = p
    ...
    # Same geometry, but MI applied on full batch globally
    p = copy.deepcopy(best_params)
    p['mi_mode'] = 'global_batch'
    p['mi_use_schedule'] = True
    configs['global_mi'] = p

    # Proposed local MI, but active from the start
    p = copy.deepcopy(best_params)
    p['mi_mode'] = 'local_pos'
    p['mi_use_schedule'] = False
    p['mi_start_epoch'] = 0
    configs['local_mi_no_schedule'] = p

    # MI without contrastive geometry: global version
    p = copy.deepcopy(best_params)
    p['use_contrastive'] = False
    p['pair_mode'] = 'none'
    p['mi_mode'] = 'global_batch'
    p['mi_use_schedule'] = True
    configs['mi_only_global'] = p

    # MI without contrastive geometry: local version on random neighborhoods
    p = copy.deepcopy(best_params)
    p['use_contrastive'] = False
    p['pair_mode'] = 'dynamic_ite'
    p['mi_mode'] = 'local_pos'
    p['mi_use_schedule'] = True
    configs['mi_only_local'] = p

    # Optional sanity check: local MI but with random pairs rather than effect-aware pairs
    p = copy.deepcopy(best_params)
    p['pair_mode'] = 'random'
    p['mi_mode'] = 'local_pos'
    p['mi_use_schedule'] = True
    configs['local_mi_random_pairs'] = p
